...
variable "root_mode" {
  type        = string
  default     = "create"
  description = "content.mode for the quod-root group. `create` ONLY for the FIRST deploy that founds genesis. After the namespace exists (and ESPECIALLY after the committee has grown past N=1), re-deploy with `-var root_mode=join -var genesis_hash=<hex>`: a `create` node whose CSI volume is ever wiped/re-provisioned would silently RE-FOUND a DIVERGENT genesis (a catastrophic fork), whereas `join` resumes from the volume if present and otherwise trustlessly catches up against the pinned anchor — same as every other member. The homogeneous end-state is EVERY node mode=join; `create` is a one-shot bootstrap. Requires genesis_hash when set to join."
}

# ============================================================================
# quod — quod:root DispersedSimplex deploy: one founder + optional joiners.
#
#  - group "quod-root" (count 1) is the BOOTSTRAP node. mode=${var.root_mode}:
...
#   1. nomad job run deploy/quod.nomad                 # founder only (create, join_count=0)
#   2. nomad alloc logs <quod-root-alloc> | grep 'genesis anchor'   # copy the hex
#   3. nomad job run -var join_count=N -var genesis_hash=<hex> deploy/quod.nomad
