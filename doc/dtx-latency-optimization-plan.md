...
| `quod_catchup:MAX_INFLIGHT` (removed in the in-progress Phase-1A cut) | was 32 read workers per hosted ontology | excess certified-history pulls were silently dropped, leaving the caller to its deadline | The earlier unchanged-wire/default-32 proposal is superseded by [catchup-page-credit-plan.md](catchup-page-credit-plan.md): one grant per authenticated link, existing producer-owned FIFO rows, reader retirement on DOWN and ordered send acceptance, no ontology-wide worker cap. This is a coordinated catch-up wire change, not yet deployed. |
