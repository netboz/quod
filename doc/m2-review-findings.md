...
All four `raft_safety_SUITE` multi-node tests, 18 `quod_log_tests`, and the full 96-test
eunit suite pass after these fixes.

## Real bugs (live in M2)

1. **[FIXED] `quod_log.erl` — chunked AppendEntries unconditionally dropped → replication
...
2. **[FIXED] `quod_conn.erl` / `quod_log.erl` — dead-but-dialable member → unbounded outbox.**
   A connection that never comes up (`quic:connect` timeout/closed) discarded its queued
   `{open_link, …}`, so the waiter got neither `link_up` nor `link_error`; `quod_log`'s
...
3. **[FIXED] `quod_log.erl` — multi-voter restart marked `quod_prolog` ready over an empty
...
4. **[FIXED] `quod_log.erl` / `quod_prolog.erl` — synchronous `append`↔`apply_block` deadlock.**
   `quod_prolog:submit_write` calls `quod_log:append` synchronously while `quod_log`'s
   ...
   `quod_log` never blocks on `quod_prolog`, so it always stays free to service `append`.
   The OCC verdict goes straight to the parked client; a forward gap asks `quod_log` to
...
5. **[DOC] `quod_log.erl` — a silently half-open cached link is never recovered.** If a
...
6. **[FIXED] `quod_log.erl:apply_committed` — badmatch if `I ≤ snap_idx`.** `entry_at/2`
...
7. **[FIXED] `quod_log.erl:reassemble` — duplicate chunk `Seq` double-counted bytes.** A
...
8. **[FIXED] `quod_log_store.erl:scan_log` — silent truncation past mid-log corruption.**
...
- **[FIXED] `quod_log.erl:send_raft` — duplicate `link_up` closed the LIVE link (HIGH).** send_raft
...
- **[FIXED] chunk path (MEDIUM) — inbound guard `?CHUNK_BYTES + 64` dropped a full chunk frame** (the
  envelope + Ns + ref overflow it); a single oversized `#change` can chunk in M2. → guard `+ 1024`, and a
...
- **[NOTE] `encode/1`+`decode/1` mirror quod_brahms's** — left as-is: trivial `term_to_binary` wrappers, and
  quod_log's codec has diverged (`decode_record` without `[safe]`); a shared module would over-abstract two one-liners.
