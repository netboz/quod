// Isolated B revision: owner-observed coordinator attempts, not coordinator_total.
// node analyze-campaign-B.mjs OUTPUT.json RESULTS INVENTORY MANIFEST [...]
...
export function analyze(input,results,inventory){
  ...
  const coordinates=spans.filter(s=>s.name==='quod.dtx.coordinate').map(s=>{
    ...
    const stages=[],events=s.events.filter(e=>e.name==='dtx.stage'||
      (duration_scope===LEGACY_SCOPE&&e.name==='dtx.completed'));
    ...
    if(!owned&&!events.some(e=>e.name==='dtx.completed'))issues.push({kind:'missing_legacy_group_close_event',key:key(s)});
    ...
    return{span:s,stages:duration_scope==='unrecognized_duration_scope'?[]:stages,duration_scope,...ancestry,
  ...
  return{schema:'quod.campaign-attribution/coordinator-B-v3',requests,issues,queue,
