...
for pair in "waves:$WAVES" "requests-per-node:$REQUESTS_PER_NODE" \
            ...
            "max-attempts:$MAX_ATTEMPTS" "retry-delay-ms:$RETRY_DELAY_MS"; do
...
run_request() {
  ...
  detail=${detail//$'\t'/ }
  detail=${detail//$'\n'/ }
  printf '%s\t%d\t%s\t%s\t%s\t%d\t%d\t%d\t%d\n' \
    "$endpoint" "$rc" "$code" "$elapsed_s" "$detail" "$attempt" \
    "$retry_retry" "$retry_busy" "$retry_other" > "$output"
  rm -f "$response"
}
...
# so every reader MUST split on tab only (-F'\t'), or later columns shift.
...
echo "  retries:   retry=$RETRY_RETRY busy=$RETRY_BUSY other=$RETRY_OTHER"
