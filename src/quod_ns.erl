...
(consensus + durable block log), `m:quod_prolog` (the KB projection),
`m:quod_catchup` (trustless catch-up endpoint), `m:quod_feed` (the dissemination
feed — gossips committed blocks to the crowd), and `m:quod_runtime` (the derived
...
from the reloaded log; the catch-up/feed endpoints re-subscribe their channels); if a
...
init({Ns, Config}) ->
    ...
    Children =
        [#{id => quod_simplex, start => {quod_simplex, start_link, [Ns, Config]},
         ...
         %% from a READ-ONLY store view (it opens its own fd; never touches the writer's handle).
         %% It depends on the durable log but holds no state the earlier children need.
