...
ontology, serves `prove/3`, and applies committed blocks from `quod_log` in log
...
- **Writes** (a proof that staged asserts/retracts) become a `#change{}` submitted
  to `quod_log`; the caller is parked and replied to when the block applies (or
  reaped by a per-tx TTL if the verdict never arrives).
- **`apply_block/3`** is the deterministic state machine `quod_log` drives on every
...
init({Ns, Config}) ->
    ...
    %% Ask quod_log (already up under the per-ns sub-sup) to replay committed blocks
    %% into this fresh kb; it casts mark_ready when the kb is caught up. Async, so
    %% init does not block on a callback.
    case quod_reg:where({quod_log, Ns}) of
        undefined -> ok;
        _Pid      -> catch quod_log:rebuild(Ns)
    end,
    {ok, S}.
...
%% quod_log; the verdict is delivered via apply_block (correlated by tx_id) or the
...
submit_write(From, Bindings, Diff, ReadSet, CallerNs, S = #s{ns = Ns}) when CallerNs =:= Ns ->
    Tx     = tx_id(S#s.self),
    Change = #change{tx_id = Tx, caller_ns = CallerNs, diff = Diff,
    ...
    case quod_log:append(Ns, Change) of
