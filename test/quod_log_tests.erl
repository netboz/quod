-module(quod_log_tests).
-moduledoc """
Pure-logic unit tests for `quod_log`'s safety-critical Raft helpers — the parts that
...
quorum_test() ->
    ?assertEqual(1, quod_log:quorum(quod_log:mk_d(#{snap_cfg => [?A]}))),
    ?assertEqual(2, quod_log:quorum(quod_log:mk_d(#{snap_cfg => [?A, ?B, ?C]}))),
    ?assertEqual(3, quod_log:quorum(quod_log:mk_d(#{snap_cfg => [?A, ?B, ?C, {"d", 4}, {"e", 5}]}))).

derive_committee_base_test() ->
    D = quod_log:mk_d(#{snap_cfg => [?A, ?B, ?C]}),
    ?assertEqual([?A, ?B, ?C], lists:sort(quod_log:derive_committee(D))).
...
derive_committee_add_remove_test() ->
    ...
    D = quod_log:mk_d(#{snap_cfg => [], log => Log}),
    ?assertEqual([?A, ?C], lists:sort(quod_log:derive_committee(D))).
...
up_to_date_test() ->
    ...
    ?assert(quod_log:up_to_date(2, 1, 1, 9)),        %% higher term wins regardless of index
    ?assertNot(quod_log:up_to_date(1, 9, 2, 1)),     %% lower term loses regardless of index
    ?assert(quod_log:up_to_date(2, 5, 2, 5)),        %% equal term+index ⇒ up to date (>=)
    ?assert(quod_log:up_to_date(2, 6, 2, 5)),        %% equal term, longer log wins
    ?assertNot(quod_log:up_to_date(2, 4, 2, 5)).     %% equal term, shorter log loses
...
term_at_test() ->
    D = quod_log:mk_d(#{log => [ent(1, 1), ent(2, 1), ent(3, 2)]}),
    ?assertEqual(0, quod_log:term_at(0, D)),         %% index-0 sentinel
    ?assertEqual(1, quod_log:term_at(1, D)),
    ?assertEqual(2, quod_log:term_at(3, D)),
    ?assertEqual(undefined, quod_log:term_at(4, D)).

term_at_snapshot_sentinel_test() ->
    D = quod_log:mk_d(#{snap_idx => 7, snap_term => 3, log => []}),
    ?assertEqual(3, quod_log:term_at(7, D)),         %% snapshot point
    ?assertEqual(7, quod_log:last_log_index(D)),     %% empty log ⇒ snapshot index/term
    ?assertEqual(3, quod_log:last_log_term(D)).

last_log_nonempty_test() ->
    D = quod_log:mk_d(#{log => [ent(1, 1), ent(2, 4)]}),
    ?assertEqual(2, quod_log:last_log_index(D)),
    ?assertEqual(4, quod_log:last_log_term(D)).
...
truncate_append_pure_append_test() ->
    D = quod_log:mk_d(#{log => [ent(1, 1), ent(2, 1)]}),
    {NewLog, Action} = quod_log:truncate_append(D, [ent(3, 2), ent(4, 2)]),
...
truncate_append_idempotent_skip_test() ->
    ...
    D = quod_log:mk_d(#{log => [ent(1, 1), ent(2, 1), ent(3, 1)]}),
    {NewLog, Action} = quod_log:truncate_append(D, [ent(2, 1), ent(3, 1)]),
...
truncate_append_conflict_test() ->
    ...
    D = quod_log:mk_d(#{log => [ent(1, 1), ent(2, 1), ent(3, 1)]}),
    {NewLog, Action} = quod_log:truncate_append(D, [ent(2, 2), ent(3, 2)]),
...
truncate_append_partial_overlap_test() ->
    ...
    D = quod_log:mk_d(#{log => [ent(1, 1), ent(2, 1), ent(3, 1)]}),
    {NewLog, Action} = quod_log:truncate_append(D, [ent(2, 1), ent(3, 2), ent(4, 2)]),
...
truncate_append_heartbeat_test() ->
    D = quod_log:mk_d(#{log => [ent(1, 1)]}),
    ?assertEqual({[ent(1, 1)], noop}, quod_log:truncate_append(D, [])).
...
figure8_prior_term_not_committed_test() ->
    ...
    D = quod_log:mk_d(#{self => ?A, snap_cfg => [?A, ?B, ?C], cur_term => 2,
                        commit_index => 0, log => Log,
                        match_index => #{?B => 2, ?C => 0}}),   %% B holds 1,2 but NOT 3
    ?assertEqual(0, commit_index_of(quod_log:advance_commit(D))).
...
figure8_current_term_commits_test() ->
    Log = [ent(1, 1), ent(2, 1), ent(3, 2)],
    D0 = quod_log:mk_d(#{self => ?A, snap_cfg => [?A, ?B, ?C], cur_term => 2,
                         commit_index => 0, log => Log,
                         match_index => #{?B => 3, ?C => 0}}),   %% B now holds up to 3
    ?assertEqual(3, commit_index_of(quod_log:advance_commit(D0))).
...
current_term_majority_commits_test() ->
    ...
    D0 = quod_log:mk_d(#{self => ?A, snap_cfg => [?A, ?B, ?C], cur_term => 2,
                         commit_index => 0, log => Log,
                         match_index => #{?B => 2, ?C => 0}}),
    ?assertEqual(2, commit_index_of(quod_log:advance_commit(D0))).
...
no_majority_no_commit_test() ->
    ...
    ?assertEqual(0, commit_index_of(quod_log:advance_commit(D0))).
...
codec_roundtrip_test() ->
    ...
    ?assertEqual(RV, quod_log:decode(quod_log:encode(RV))),
    AE = #append_entries{term = 3, leader_id = ?A, prev_log_index = 4, prev_log_term = 2,
                         entries = [ent(5, 3)], leader_commit = 4},
    ?assertEqual(AE, quod_log:decode(quod_log:encode(AE))).

codec_garbage_is_safe_test() ->
    ?assertEqual(error, quod_log:decode(<<"not-a-term">>)).

%% #d{} is private; read commit_index back via the TEST accessor.
commit_index_of(D) -> quod_log:commit_index(D).
